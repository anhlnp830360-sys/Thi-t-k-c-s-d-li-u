...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{imagesssms_create_db.png}
...
\noindent Ở bảng SERVER sẽ được lấy dữ liệu từ file csv.
\begin{itemize}
...
\noindent Ở bảng TELEMETRY sẽ được lấy dữ liệu từ file csv.
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Lý do nạp cuối cùng:} Bảng này ghi vết lại lịch sử thao tác của USER tác động lên các đối tượng khác trong suốt quá trình vận hành, do đó cần nạp sau cùng để đảm bảo có đầy đủ dữ liệu tham chiếu.
\end{itemize}
...
\begin{center}
\small
\begin{longtable}{|>{\raggedright\arraybackslash}p{1.2cm}|>{\raggedright\arraybackslash}p{2.2cm}|>{\raggedright\arraybackslash}p{3.0cm}|>{\raggedright\arraybackslash}p{3.2cm}|>{\raggedright\arraybackslash}p{3.2cm}|>{\raggedright\arraybackslash}p{1.2cm}|}
\caption{Bảng Ma trận Kiểm thử và Xác nhận Dữ liệu} \label{tab:test_cases_chuong4} \\
\hline
\textbf{Mã TC} & \textbf{Hạng mục kiểm thử} & \textbf{Nội dung kiểm tra (Test Case)} & \textbf{Câu lệnh / Thao tác kiểm thử} & \textbf{Kết quả mong đợi} & \textbf{Trạng thái} \\
\hline
\endfirsthead

\multicolumn{6}{c}{\small\textit{(Bảng \thetable{} -- tiếp theo)}} \\
\hline
\textbf{Mã TC} & \textbf{Hạng mục kiểm thử} & \textbf{Nội dung kiểm tra (Test Case)} & \textbf{Câu lệnh / Thao tác kiểm thử} & \textbf{Kết quả mong đợi} & \textbf{Trạng thái} \\
\hline
\endhead
...
TC-02 & Cấu trúc Bảng & Kiểm tra đủ 16 bảng theo thiết kế & \texttt{SELECT COUNT(*) FROM sys.tables;} & Xuất hiện đủ 16 bảng, đúng tên và cấu trúc & Đạt \\ \hline
TC-03 & Ràng buộc PK & Kiểm tra tính duy nhất và không NULL của Khóa chính & Thử INSERT 2 bản ghi trùng PK vào bảng [USER] & Hệ thống báo lỗi trùng lặp PK & Đạt \\ \hline
TC-04 & Ràng buộc FK & Kiểm tra quan hệ cha-con & Thử INSERT bản ghi con có FK không tồn tại & Hệ thống từ chối do vi phạm khóa ngoại & Đạt \\ \hline
TC-05 & Ràng buộc NOT NULL & Thử bỏ trống các trường bắt buộc & Thử INSERT giá trị NULL vào cột NOT NULL & Hệ thống báo lỗi và từ chối lưu dữ liệu & Đạt \\ \hline
TC-06 & Ràng buộc UNIQUE & Kiểm tra tính duy nhất của username, email, server\_name, IP & Thử thêm 2 Server có cùng địa chỉ IP & SQL Server chặn hành động vi phạm UNIQUE & Đạt \\ \hline
TC-07 & Ràng buộc DEFAULT & Thêm dữ liệu mới không truyền trường DEFAULT & Thử INSERT [USER] không nhập created\_at & Hệ thống tự động điền ngày giờ hiện tại & Đạt \\ \hline
TC-08 & Ràng buộc CHECK & Kiểm tra miền giá trị status, severity & Thử gán severity = 'SUPER\_CRITICAL' hoặc cpu\_usage = 150 & Hệ thống báo lỗi vi phạm điều kiện CHECK & Đạt \\ \hline
...
TC-13 & Ngưỡng Cảnh báo & Kiểm tra ngưỡng Warning/Critical của từng Server & Lọc bảng ALERT\_THRESHOLD theo từng chỉ số & Ngưỡng hợp lệ, không bị trùng lặp metric & Đạt \\ \hline
...
TC-17 & Lịch sử Maintenance & Kiểm tra hoạt động bảo trì gắn với Ticket & Lọc bảng MAINTENANCE theo ticket\_id\_fk & Lưu đầy đủ loại bảo trì, thời gian, kết quả & Đạt \\ \hline
TC-18 & Nhật ký Audit Log & Kiểm tra lịch sử thao tác của người dùng được tự động ghi nhận & Thay đổi trạng thái Server/Ticket và kiểm tra AUDIT\_LOG & Trigger hoạt động tốt, tự động lưu vết hành động & Đạt \\ \hline
TC-19 & Dữ liệu mẫu & Đối chiếu số lượng và chất lượng dữ liệu nạp ban đầu & SELECT COUNT(*) trên từng bảng & Dữ liệu mẫu nạp đủ, không bị lỗi quan hệ & Đạt \\ \hline
TC-20 & Truy vấn Nghiệp vụ & Chạy các câu lệnh SQL đối chiếu kết quả thực tế & Thực thi các câu SQL nghiệp vụ & Kết quả trả về chính xác, tốc độ truy vấn tối ưu & Đạt \\ \hline
\end{longtable}
\end{center}
