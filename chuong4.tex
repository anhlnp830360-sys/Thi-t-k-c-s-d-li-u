...
\caption{Ma trận kiểm thử và xác nhận dữ liệu}
...
\textbf{Mã TC}
&
\textbf{Hạng mục kiểm thử}
&
\textbf{Nội dung kiểm tra}
&
\textbf{Thao tác kiểm thử}
&
...
Khởi tạo Database
&
Kiểm tra Database \texttt{AR\_IMMS} đã được tạo và có thể truy cập.
&
\texttt{SELECT db\_id('AR\_IMMS');}
&
Database tồn tại và hoạt động bình thường.
&
Đạt
\\ \hline
...
Cấu trúc bảng
&
Kiểm tra đủ 16 bảng theo thiết kế
(\texttt{USER}, \texttt{SERVER}, \texttt{TELEMETRY}, \ldots).
&
\texttt{SELECT COUNT(*) FROM sys.tables;}
&
Xuất hiện đủ 16 bảng, đúng tên và cấu trúc.
&
Đạt
\\ \hline
...
Ràng buộc PK
&
Kiểm tra tính duy nhất và không NULL của khóa chính (Primary Key).
&
Thử \texttt{INSERT} 2 bản ghi trùng PK vào bảng \texttt{USER}.
&
Hệ thống báo lỗi trùng lặp PK
(\texttt{PRIMARY KEY constraint}).
&
Đạt
\\ \hline
...
(\texttt{ROOM} $\rightarrow$ \texttt{SITE},
\texttt{SERVER} $\rightarrow$ \texttt{RACK}, \ldots).
&
Thử \texttt{INSERT} bản ghi con có FK tham chiếu đến ID không tồn tại.
&
Hệ thống từ chối do vi phạm khóa ngoại
(\texttt{FOREIGN KEY constraint}).
&
Đạt
\\ \hline
...
Ràng buộc NOT NULL
&
Kiểm tra các trường bắt buộc như \texttt{username}, \texttt{ip\_address}, \ldots.
&
Thử \texttt{INSERT} giá trị NULL vào cột có cấu hình \texttt{NOT NULL}.
&
Hệ thống báo lỗi và từ chối lưu dữ liệu.
&
Đạt
\\ \hline
...
Ràng buộc UNIQUE
&
Kiểm tra tính duy nhất của \texttt{username}, \texttt{email}, \texttt{server\_name} và IP.
&
Thử thêm 2 Server có cùng địa chỉ IP.
&
SQL Server chặn hành động vi phạm
...
Ràng buộc DEFAULT
&
Kiểm tra các trường được cấu hình giá trị mặc định.
&
Thử \texttt{INSERT} \texttt{USER} mà không nhập giá trị cho trường có \texttt{DEFAULT}.
&
Hệ thống tự động điền giá trị mặc định theo cấu hình.
&
Đạt
\\ \hline
...
Ràng buộc CHECK
&
Kiểm tra miền giá trị của \texttt{status}, \texttt{severity} và các chỉ số giám sát.
&
Thử gán \texttt{severity = 'SUPER\_CRITICAL'} hoặc
\texttt{cpu\_usage = 150}.
&
Hệ thống báo lỗi vi phạm điều kiện
\texttt{CHECK constraint}.
&
Đạt
\\ \hline
...
Hạ tầng không gian
&
Kiểm tra liên kết phân cấp
Site $\rightarrow$ Room $\rightarrow$ Rack $\rightarrow$ Server.
&
Thực hiện truy vấn \texttt{JOIN} từ \texttt{SITE} đến \texttt{SERVER}.
&
Cấu trúc phân cấp đúng, không có dữ liệu sai lệch.
&
Đạt
\\ \hline
...
Quản lý Container
&
Kiểm tra Container được gắn đúng Server và có trạng thái hợp lệ.
&
Tra cứu Container theo \texttt{server\_id\_fk}.
&
Không có Container mồ côi; trạng thái hiển thị chính xác.
&
Đạt
\\ \hline
...
Bảo hành Warranty
&
Kiểm tra ngày bắt đầu, ngày kết thúc và quan hệ Server--Warranty.
&
Kiểm tra điều kiện
\texttt{warranty\_end >= warranty\_start}.
&
Ngày kết thúc sau ngày bắt đầu; mỗi Server được gán một bảo hành.
&
Đạt
\\ \hline
...
Dữ liệu Telemetry
&
Kiểm tra dữ liệu CPU, RAM, Disk, nhiệt độ và mốc thời gian.
&
Lọc các bản ghi Telemetry theo \texttt{server\_id}.
&
Dữ liệu giám sát nằm trong khoảng cho phép và thuộc đúng Server.
&
Đạt
\\ \hline
...
Ngưỡng cảnh báo
&
Kiểm tra ngưỡng Warning/Critical của từng Server.
&
Lọc bảng \texttt{ALERT\_THRESHOLD} theo từng chỉ số.
&
Ngưỡng hợp lệ, không bị trùng metric trên cùng một Server.
&
Đạt
\\ \hline
...
Phát sinh Alert
&
Kiểm tra Alert được tạo khi dữ liệu giám sát vượt ngưỡng.
&
Kiểm tra các Alert mới được tạo tự động.
&
Alert phản ánh đúng mức độ nghiêm trọng và
\texttt{server\_id} có sự cố.
&
Đạt
\\ \hline
...
Quản lý Incident
&
Kiểm tra Incident liên kết với Alert.
&
Thử tạo Incident từ một \texttt{alert\_id}.
&
Incident chỉ được khởi tạo khi chứa
\texttt{alert\_id} hợp lệ.
&
Đạt
\\ \hline
...
Phân công Ticket
&
Kiểm tra Ticket thuộc Incident và được phân công cho Kỹ thuật viên.
&
Tra cứu \texttt{TICKET} liên kết với \texttt{INCIDENT} và \texttt{USER}.
&
Thông tin sự cố, Ticket và người xử lý chính xác.
&
Đạt
\\ \hline
...
Lịch sử Maintenance
&
Kiểm tra hoạt động bảo trì được gắn với Ticket.
&
Lọc bảng \texttt{MAINTENANCE} theo
\texttt{ticket\_id\_fk}.
&
Lưu đầy đủ loại bảo trì, thời gian, kết quả và Ticket liên quan.
&
Đạt
\\ \hline
...
Nhật ký Audit Log
&
Kiểm tra lịch sử thao tác của người dùng được ghi nhận.
&
Thay đổi trạng thái Server/Ticket và kiểm tra
\texttt{AUDIT\_LOG}.
&
Trigger hoạt động đúng, tự động lưu vết hành động và thời gian.
&
Đạt
\\ \hline
...
Dữ liệu mẫu (Seed)
&
Đối chiếu số lượng và chất lượng dữ liệu nạp ban đầu.
&
Thực hiện \texttt{SELECT COUNT(*)} trên từng bảng.
&
Dữ liệu mẫu được nạp đầy đủ, không lỗi quan hệ hoặc thiếu trường.
&
Đạt
\\ \hline
...
Truy vấn nghiệp vụ
&
Thực hiện 19 câu lệnh SQL tại mục 4.7.1 và đối chiếu kết quả thực tế.
&
Thực thi các câu SQL từ Câu 4.7.1.1 đến Câu 4.7.1.19.
&
Kết quả trả về chính xác và thời gian truy vấn đáp ứng yêu cầu.
&
Đạt
\\ \hline
